...
> **This repo is `ThachThanhThien/clean-architecture`** — so `SLUG` = `clean-architecture`, and the finished course goes live at
> `https://thachthanhthien.github.io/clean-architecture/`.

Designing maintainable systems with Clean Architecture — the Dependency Rule; entities, use cases, interface adapters, and frameworks/drivers layers; ports & adapters (hexagonal architecture); boundaries and dependency inversion; and keeping business logic independent of UI, database, and frameworks.

**Notes for this topic**
- Sibling to the **Design Patterns** and **System Design** courses — reference them, don’t duplicate. **PRISM_LANGS**: one readable language (e.g. `typescript`) + `bash`/`json`/`text`. Category: **Architecture** (or **Software Design**).
- ⚠ **Attribute precisely:** **Clean Architecture** (Robert C. Martin), **Hexagonal / Ports & Adapters** (Alistair Cockburn), and **Onion Architecture** (Jeffrey Palermo) are related but **distinct** — don’t conflate them; and DIP ≠ DI ≠ IoC.
